...
const CheckPrerequisites = () => {
  ...
  const [retakeImpact, setRetakeImpact] = useState(false);
  const [message, setMessage] = useState('');
  const [results, setResults] = useState(null);
  const [availableCourses, setAvailableCourses] = useState([]);
  const [isLoading, setIsLoading] = useState(false);
  const [newCourse, setNewCourse] = useState({ code: '', grade: '' });
  ...
  const majors = {
    "computer_science": [
     ...
     multimedia_computing: [
      { "id": 1, "name": "Intro to Programming using Java or Java for Programmers", "options": ["Introduction to Programming using Java", "Java for Programmers"], "requirements": ["CISC 1115", "CISC 1170"], "prerequisites": [ "For CISC 1115, a student must not receive credit for CISC 1110 (Introduction to Programming using C++). Otherwise if they receive credit for CISC 1110 they must take CISC 1170"] },
      { "id": 2, "name": 'Core Multimedia Computing Courses', "options": ["Introduction to Multimedia Computing", "Introduction to Discrete Structures", "Computer and Ethics", "Introduction to Modern Programming Techniques", "Data Structures", "Analysis of Algorithms" ,"Operating Systems", "Computer Graphics", "Multimedia Computing"], "requirements": ["CISC 1600", "CISC 2210", "CISC 2820W", "CISC 3115", "CISC 3130", "CISC 3220", "CISC 3320", "CISC 3620", "CISC 3630"], "prerequisites": ['There is no prerequisite for CISC 1600. If a student wishes to take CISC 2820W they must CISC 1115 or CISC 1110 with a C or higher and ENGL 1012 with a C or higher. If a student wishes to take CISC 3115 they must pass CISC 1115 with a C or higher otherwise if they have taken CISC 3110 then they are exempt from this course. If a student wishes to take CISC 3130 they must pass CISC 1115 or CISC 1110 with a C or higher and CISC 3115 or CISC 3110 with a C or higher. If a student wishes to take CISC 3220 they must pass CISC 2210 and CISC 3130 with a C or higher. If a student wishes to take CISC 3320 they must pass both CISC 3130 and CISC 3310 with a C or higher'] },
  ...
  useEffect(() => {
    ...
    if (major && majors[major]) {
      majors[major].forEach(requirement => {
        // For each requirement, add all its required courses
        requirement.requirements.forEach(courseCode => {
          courses.push({
            code: courseCode,
            name: requirement.options.find(opt => opt.includes(courseCode)) || requirement.name,
    ...
    if (minor && minors[minor]) {
      minors[minor].forEach(requirement => {
        requirement.requirements.forEach(courseCode => {
          courses.push({
            code: courseCode,
            name: requirement.options.find(opt => opt.includes(courseCode)) || requirement.name,
  ...
  const isGradeSufficient = (actualGrade, minGrade) => {
    const gradeOrder = ['A', 'A-', 'B+', 'B', 'B-', 'C+', 'C', 'C-', 'D+', 'D', 'F'];
    if (actualGrade === 'P') return true;
    if (actualGrade === 'NP' || actualGrade === 'W') return false;
    return gradeOrder.indexOf(actualGrade) <= gradeOrder.indexOf(minGrade);
  };

  const checkPrerequisites = (courseCode) => {
    ...
    // Check each prerequisite text block
    course.prerequisites.forEach(prereqText => {
      // Extract minimum grade needed (default to C if not specified)
      const gradeMatch = /with a ([A-Z][+-]?) or higher/.exec(prereqText);
      const minGrade = gradeMatch ? gradeMatch[1] : 'C';
      
      // Find all course codes mentioned in prerequisites
      const courseCodes = prereqText.match(/(CISC|MATH|PHIL|BUSN|ECON|ACCT|FIN|PYSCH|SOCY|POLS|FILM|TVRA|MUSC|ARTD)\s\d+/g) || [];
  
      courseCodes.forEach(reqCourse => {
        const hasCourse = courseHistory.some(ch => ch.code === reqCourse);
        const isTaking = currentlyTaking.includes(reqCourse);
        
        if (hasCourse) {
          const gradeInfo = courseHistory.find(ch => ch.code === reqCourse);
          const gradeMet = isGradeSufficient(gradeInfo.grade, minGrade);
          
          results.details.push({
            met: gradeMet,
            message: gradeMet 
              ? `✓ Met ${reqCourse} with grade ${gradeInfo.grade} (needed ${minGrade}+)`
              : `✗ Grade too low in ${reqCourse} (${gradeInfo.grade}, needed ${minGrade}+)`
          });
          
          if (!gradeMet) results.met = false;
          
        } else if (isTaking) {
          results.details.push({
            met: true,
            message: `Currently taking ${reqCourse} (assuming you'll pass)`
          });
        } else {
          results.details.push({
            met: false,
            message: `✗ Missing prerequisite: ${reqCourse} (needs ${minGrade}+)`
          });
          results.met = false;
        }
      });
  
      // Check for forbidden courses
      if (prereqText.includes('must not have credit for') || prereqText.includes('must not receive credit for')) {
        const forbiddenCourseMatch = prereqText.match(/(?:must not have credit for|must not receive credit for)\s+(\w+\s\d+)/);
        if (forbiddenCourseMatch) {
          const forbiddenCourse = forbiddenCourseMatch[1];
          const hasForbidden = courseHistory.some(ch => ch.code === forbiddenCourse);
          
          results.details.push({
            met: !hasForbidden,
            message: hasForbidden 
              ? `✗ Cannot take ${courseCode} because you've completed ${forbiddenCourse}`
              : `✓ No conflict with forbidden course ${forbiddenCourse}`
          });
          
          if (hasForbidden) results.met = false;
        }
      }
    });
  ...
  const addCourseToHistory = () => {
    if (newCourse.code && newCourse.grade) {
      setCourseHistory([...courseHistory, newCourse]);
      setNewCourse({ code: '', grade: '' });
    }
  };

  const removeCourseFromHistory = (courseCode) => {
    setCourseHistory(courseHistory.filter(c => c.code !== courseCode));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setIsLoading(true);
      const checkResults = checkPrerequisites(course);
      setResults(checkResults);
      setMessage(
        checkResults.met
          ? `All prerequisites met for ${checkResults.courseName}!`
          : `Prerequisites not met for ${checkResults.courseName}`
      );
    } catch (error) {
        ...
        <div className="form-row">
          <div className="form-group">
            ...
            >
              ...
              {Object.keys(majors).map(majorKey => (
                <option key={`major-${majorKey}`} value={majorKey}>
                  {majorKey.replace(/_/g, ' ')}
                </option>
              ))}
            </select>
          </div>
          ...
          <div className="form-group">
            ...
            >
              ...
              {Object.keys(minors).map(minorKey => (
                <option key={`minor-${minorKey}`} value={minorKey}>
                  {minorKey.replace(/_/g, ' ')}
                </option>
              ))}
            </select>
          </div>
...
>
  ...
  {availableCourses.map(c => (
    <option key={`course-${c.code}`} value={c.code}>
      {c.code} - {c.name}
    </option>
  ))}
</select>
        ...
        <div className="course-history-section">
          ...
          <div className="currently-taking">
            ...
            {availableCourses.map(course => (
              <label key={`taking-${course.code}`} className="checkbox-label">
                <input
                  type="checkbox"
                  checked={currentlyTaking.includes(course.code)}
                  onChange={(e) => {
                    if (e.target.checked) {
                      setCurrentlyTaking([...currentlyTaking, course.code]);
                    } else {
                      setCurrentlyTaking(currentlyTaking.filter(c => c !== course.code));
                    }
                  }}
                />
              ...
              <ul>
                {courseHistory.map((course, index) => (
                  <li key={`history-${index}`}>
                    ...
                    <button 
                      type="button" 
                      onClick={() => removeCourseFromHistory(course.code)}
        ...
        <button type="submit" disabled={isLoading} className="submit-btn">
          {isLoading ? 'Checking...' : 'Check Prerequisites'}
        </button>
      </form>

      {message && (
        <div className={`message ${results?.met ? 'success' : 'error'}`}>
          {message}
        </div>
      )}
      ...
      {results && (
        <div className="results">
          <h3>Prerequisites for {results.courseName}</h3>
          <ul>
            {results.details.map((detail, i) => (
              <li key={`result-${i}`} className={detail.met ? 'met' : 'unmet'}>
                {detail.message}
              </li>
            ))}
          </ul>
